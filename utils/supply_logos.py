"""
Funções auxiliares para estatísticas de logos da BU Supply
"""
import time
from utils.db import get_db_connection_context
from psycopg2.extras import RealDictCursor

# Valor base fixo até 30/11
BASE_LOGOS_NOVEMBER = 734
TARGET_LOGOS_YEAR = 800

# Cache em memória para os dados de logos Supply
_supply_logos_cache = {
    'data': None,
    'timestamp': None,
...
def get_supply_logos_stats(force_refresh=False):
    """
    Busca estatísticas de logos da BU Supply (New + Expansão) para dezembro
    Retorna dados agregados por produto e totais
    Implementa cache em memória para evitar queries excessivas
    
    Args:
        force_refresh: Se True, ignora o cache e executa a query novamente
    
    Returns:
        dict com estatísticas de logos ou None
    """
    # Verifica cache primeiro (a menos que force_refresh seja True)
    if not force_refresh and _supply_logos_cache['data'] and _supply_logos_cache['timestamp']:
        elapsed = time.time() - _supply_logos_cache['timestamp']
        if elapsed < _supply_logos_cache['cache_duration']:
            print(f"✅ Usando cache de logos Supply (válido por mais {int(_supply_logos_cache['cache_duration'] - elapsed)}s)")
            return _supply_logos_cache['data']
    ...
    with get_db_connection_context() as conn:
        ...
        try:
            ...
            query = """
            SELECT
              "source"."Line Items - hs_object_id__produto_principal" AS "produto_principal",
            ...
            for row in results:
                produto = row.get('produto_principal') or 'N/A'
                qtd_new = row.get('qtd_new', 0) or 0
                qtd_expansao = row.get('qtd_expansao', 0) or 0
                total = qtd_new + qtd_expansao
                ...
                by_product.append({
                    'product': produto,
                    'new': qtd_new,
                    'expansion': qtd_expansao,
                    'total': total
                })
            
            # Calcula totais
            total_december = total_new + total_expansao
            total_current = BASE_LOGOS_NOVEMBER + total_december
            remaining = max(0, TARGET_LOGOS_YEAR - total_current)
            percentage = (total_current / TARGET_LOGOS_YEAR * 100) if TARGET_LOGOS_YEAR > 0 else 0
            
            result = {
                'base_logos': BASE_LOGOS_NOVEMBER,
                'new_logos_december': total_new,
                'expansion_logos_december': total_expansao,
                'total_december': total_december,
                'total_current': total_current,
                'target': TARGET_LOGOS_YEAR,
                'remaining': remaining,
            ...
            # Atualiza cache
            _supply_logos_cache['data'] = result
            _supply_logos_cache['timestamp'] = time.time()
            print(f"💾 Cache de logos Supply atualizado (válido por {_supply_logos_cache['cache_duration']}s)")
            
            return result
            
        except Exception as e:
            print(f"Erro ao buscar estatísticas de logos Supply: {e}")
            import traceback
            traceback.print_exc()
            return None
        finally:
